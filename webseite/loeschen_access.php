...
$conn = new mysqli($servername, $username,$password,$dbname);
	mysqli_query($conn, "SET NAMES 'utf8'");
		...
		$sqlfull="SELECT bezeichnung, ID_Produkt FROM access";
		$sfull =mysqli_query($conn,$sqlfull );
		echo"<p class='bold'> Accessoires löschen
			<br>
			<table>";
				while($row= mysqli_fetch_assoc($sfull))
		{
				...
				echo "
					...
					<td>
					<form action='loeschen_access.php?ID=".$row["ID_Produkt"]."' method='post' >
		...
		if (isset ($_POST['entfernen']))
			{	
			$sLöschen = $_GET['ID'];
			$ssqlentfernen = "DELETE FROM access WHERE ID_Produkt=$sLöschen ";	
